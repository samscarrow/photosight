...
class AutoStraightener:
    ...
    def preview_straightening(self, 
                            image: np.ndarray, 
                            scale: float = 0.25) -> Dict:
        """
        Generate a small preview of straightening result
        
        Args:
            image: RGB image array
            scale: Scale factor for preview (0.25 = 25% size)
            
        Returns:
            Dictionary with preview and analysis results
        """
        # Resize for faster processing
        h, w = image.shape[:2]
        preview_h, preview_w = int(h * scale), int(w * scale)
        preview_image = cv2.resize(image, (preview_w, preview_h), interpolation=cv2.INTER_AREA)
        
        # Analyze the preview
        analysis = self.analyze_image(preview_image)
        ...
        if analysis['needs_straightening']:
            # Apply straightening to preview
            straightened, _ = self.apply_straightening(
                preview_image, 
                analysis['suggested_angle'],
                crop_to_valid=True
            )
            result['preview_straightened'] = straightened
            result['preview_original'] = preview_image
